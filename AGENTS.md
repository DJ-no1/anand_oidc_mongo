...
- Admin APIs expect users with `role: "admin"` on the User model; routes are under `/api/admin/*`; the Next.js `/admin` UI should stay **discreet** (no links from public marketing or IdP chrome); OAuth clients are managed under `/api/projects` (e.g. `GET/POST /api/projects`, `GET/POST /api/projects/:projectId/clients`). **Registration** persists `role` from the validated body: only **`customer`** / **`seller`** in production; **`admin`** / **`support`** are rejected by Joi (and blocked again in `auth.service` in production). For **local dev only**, set **`ALLOW_REGISTER_ADMIN_ROLES=true`** (and non-production `NODE_ENV`) to allow `admin`/`support` on `POST /api/auth/register`. Otherwise grant **`admin`** in Mongo from **`backend/`**: `pnpm run promote:admin -- you@example.com` (**`MONGODB_URI`** required). The public register UI sends **`role: "customer"`** only. Authenticated role checks use the **User** document from Mongo (including `/api/auth/me`).
